...
class MarketRepository extends ServiceEntityRepository
{
    ...
    public function getMarketDate($min, $max){
        return $this->getEntityManager()->createQuery('Select m from App\Entity\Market m where m.ValideDate > :min and m.ValideDate < :max')->setParameter('min', $min)->setParameter('max', $max)->getResult();
    }
}
